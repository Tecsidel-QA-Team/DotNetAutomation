...
{
    [TestClass]
    public class BOPlazaHost_verConfirmación_TransitosSubidos : Settingsfields_File
    {
        private static SqlConnection connection;
        private static string[] transactions = new string[2];
        private static List<string> transactionsPIds = new List<string>();
        private static List<string> transactionsHIds = new List<string>();
        private static DateTime localDate1 = DateTime.Now;
        ...
        {
            dateverTransacciones = localDate1.ToString("dd/MM/yyyy");
            localDate = localDate1.ToString("yyyyMMdd");
            ...
            try {
                ...
                {
                    Console.WriteLine("En Plaza han subido hoy: " + transactionsPIds.Count / 2);
                    ...
                    {
                        string Hour1 = transactionsHIds[0].Substring(8, 2);
                        string Min1 = transactionsHIds[0].Substring(10, 2);
                        string Sec1 = transactionsHIds[0].Substring(12, 2);
                        Console.WriteLine("En Host han subido hoy: " + transactionsHIds.Count / 2);                        
                        System.Threading.Thread.Sleep(1000);
                        ...
                        if (transResult.Count < 19)
                        {
                            ...
                            if (transRes.Equals(transactionsHIds[1]))
                            {
                                Console.WriteLine("Hay " + elementsFound.Substring(elementBg, elementlength) + " transaccion/es y el último tránsito: " + transactionsHIds[1]+ " ha subido satisfactoriamente el dia de hoy " + dateverTransacciones + "  {0}:{1}:{2}", Hour1, Min1, Sec1);
                                return;
                            }
                            else
                            {
                                ...
                                if (transRes.Equals(transactions[1])){
                                    Console.WriteLine("Hay "+elementsFound.Substring(elementBg, 2)+" transacciones y el último tránsito: "+transactionsHIds[1]+" ha sido satisfactoriamente el dia de hoy "+dateverTransacciones+" {0}:{1}:{2}",Hour1,Min1,Sec1);
                                    return;
                                }else{
                                    Console.WriteLine("No se ha subido el último tránsito a BackOffice Web desde HostManage de hoy");
                                    Assert.Fail("No se ha subido el último tránsito a BackOffice Web HostManage con fecha de hoy");
